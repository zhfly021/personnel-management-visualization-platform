package com.recruitment.utils.File;
...
public class ExcelUtil {
    ...
    public static List<Plan> readExcel(MultipartFile file){
...
//        从第一个数据开始读
        for (int i = sheet.getFirstRowNum() + 1; i <= sheet.getLastRowNum(); i++){
            ...
            //获取用人单位
            String uni = uniCell.getStringCellValue();
            //获取学科专业
            String profession = getCellValue(row.getCell(1));
            //获取岗位类别
            String job = getCellValue(row.getCell(2));
            //招聘人数
            String hireNumber = getCellValue(row.getCell(3));

            Plan plan = new Plan(uni,profession,job,Integer.parseInt(hireNumber));
